...
while 1:
    try: 
        i = int(input('Ingrese número de imagen: '))
        j = i + 1
        for num in [i-2, i-1, i, i+1]:
            if not os.path.exists(path(num)):
                raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), path(num))
        back = show_gel_percentage(i)
        palm = show_gel_percentage(j)
        text(results[0],back)
        text(results[1],palm)
        ...
        if input('Error: digite un valor adecuado. Presione "x" para salir\n>> ') == 'x': break
